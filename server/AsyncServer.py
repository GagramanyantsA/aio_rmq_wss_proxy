...
class AsyncServer:
    ...
    def __init__(self, name: str, host: str, port: int,
                 clients_controller: ClientsControllerBase, logger: Logger, exception_queue: Queue):
        self._name = name
        ...
        self._awaitable_server = websockets.serve(ws_handler=self._do_action,
                                                  host=self._host,
                                                  port=self._port,
    ...
    def start_coroutine(self) -> websockets.WebSocketServer:
        return self._awaitable_server
    ...
    async def _do_action(self, websocket: SecuredWebsocketServerProtocol, path: str):
        try:
            client_id, client_ip = self._add_client(websocket)
        except Exception as ex:
            self._logger.exception(ex)
            self._exception_queue.put((self.name, ex))
            return
